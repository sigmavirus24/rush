...
class GenericCellRatelimiter(base.BaseLimiter):
    ...
    def _gcra(
        self,
        *,
        data: typing.Optional[limit_data.LimitData],
        now: datetime.datetime,
        quantity: int,
        rate: quota.Quota,
    ) -> typing.Tuple[result.RateLimitResult, limit_data.LimitData]:
        ...
        # The theoretical arrival time defines the end-period (in the
        # future) of our bucket. We want to find, however, which is
        # later - our tat or now. Once we have that, we can find the
        # earliest point in our bucket which is the time we would start
        # allowing new requests.
        allow_at: datetime.datetime = new_tat - delay_variation_tolerance
        # Okay, we have our tat, the time we would have started allowing
        # newrequests, and present time so let's figure out the difference
        # fromour earliest point in time and now.
        distance_from_start_of_bucket = now - allow_at
        # Now that we know how far we are from the start, let's find out
        # how much we have remaining in our quota.
        ...
        if remaining < 1:
            # It's possible that distance_from_start_of_bucket is negative
            # so it is then also possible that there are fewer than 0
            # remaining requests available. In that case, we're going to be
            # ratelimited.
            remaining = 0
            limited = True
    ...
    def rate_limit(
        ...
        while attempts < MAX_CAS_ATTEMPTS:
            try:
                ratelimit_result, limitdata = self._gcra(
                    data=data, now=now, quantity=quantity, rate=rate
                )
                self.store.compare_and_swap(key=key, old=data, new=limitdata)
                return ratelimit_result
            except exceptions.CompareAndSwapError as e:
                attempts += 1
                # Reset values to current best knowledge and retry
                now = self.store.current_time()
                data = e.limitdata
                continue

        raise exceptions.CompareAndSwapError(
            "Failed to CompareAndSwap LimitData after %s attempts."
            % MAX_CAS_ATTEMPTS,
            limitdata=data,
        )
